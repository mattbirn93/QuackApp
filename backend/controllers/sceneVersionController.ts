...
export const createSceneVersion = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (prev_scene_id && mongoose.Types.ObjectId.isValid(prev_scene_id)) {
      const index = scene.sceneVersions_id_array.findIndex(
        (id) => id.toString() === prev_scene_id,
      );
      if (index !== -1) {
        scene.sceneVersions_id_array.splice(
          index + 1,
          0,
          savedSceneVersion._id as any,
        );
      } else {
...
export const updateCurrentSceneVersionContent = async (
  req: Request,
  res: Response,
) => {
  ...
  if (
    !mongoose.Types.ObjectId.isValid(sceneVersionId) ||
