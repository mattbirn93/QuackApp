...
export const fetchScenes = async (req, res) => {
    try {
...
export const fetchScenesWithVersions = async (req, res) => {
    try {
...
export const fetchScenesWithVersionContent = async (req, res) => {
    try {
        const scriptId = req.query.scriptId;
        const scenesWithVersions = await scenes
            .aggregate([
            ...
            .exec();
        console.log("Scenes with version content:", JSON.stringify(scenesWithVersions, null, 2));
        if (!scenesWithVersions || scenesWithVersions.length === 0) {
            return res
                .status(404)
                .json({ message: "No scenes found for the given script ID." });
...
export const createScene = async (req, res) => {
    ...
    try {
        ...
        savedSceneVersion.current_sceneVersionContent_id =
            savedSceneVersionContent._id;
        savedSceneVersion.sceneVersionContent_id_array.push(savedSceneVersionContent._id);
        await savedSceneVersion.save({ session });
        savedScene.sceneVersions_id_array.push(savedSceneVersion._id);
