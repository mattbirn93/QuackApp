...
export const getSceneVersionContentSocket = async (id, callback) => {
    try {
...
export const updateContentItemSocket = async (data, callback) => {
    ...
    try {
        ...
        const existingItemIndex = sceneVersionContent.content.findIndex((item) => item &&
            item.content_id &&
            item.content_id.toString() === contentItem.content_id.toString());
        if (existingItemIndex !== -1) {
            // Update existing item
            sceneVersionContent.content[existingItemIndex] = {
                ...sceneVersionContent.content[existingItemIndex],
                ...contentItem,
            };
...
export const createContentItemSocket = async (data, callback) => {
    ...
    try {
        ...
        if (prev_content_id && mongoose.Types.ObjectId.isValid(prev_content_id)) {
            const prevItemIndex = sceneVersionContent.content.findIndex((item) => item &&
                item.content_id &&
                item.content_id.toString() === prev_content_id.toString());
            if (prevItemIndex !== -1) {
            ...
            sceneVersionContent.content.unshift(contentItem);
        }
        await sceneVersionContent.save();
...
export const deleteContentItemSocket = async (data, callback) => {
    ...
    try {
        ...
        if (!sceneVersionContent.content ||
            sceneVersionContent.content.length === 0) {
            return callback("No content items found");
        }
        const existingItemIndex = sceneVersionContent.content.findIndex((item) => item && item.content_id && item.content_id.toString() === content_id);
        if (existingItemIndex === -1) {
