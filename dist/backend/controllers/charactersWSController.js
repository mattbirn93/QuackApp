...
export const getCharactersById = async (id, callback) => {
    try {
...
export const addCharacterToArray = async (data, callback) => {
    ...
    try {
        ...
        const updatedCharacters = await characters.findByIdAndUpdate(charactersId, { $push: { characters_array: newCharacter } }, { new: true, session: session });
...
export const updateCharacterInArray = async (data, callback) => {
    const { charactersId, characterId, character_name } = data;
    console.log(charactersId, characterId, character_name);
    if (!mongoose.Types.ObjectId.isValid(charactersId) ||
        !mongoose.Types.ObjectId.isValid(characterId)) {
        return callback("Invalid ID format");
    }
    try {
        const updatedCharacters = await characters.findOneAndUpdate({ _id: charactersId, "characters_array._id": characterId }, { $set: { "characters_array.$.character_name": character_name } }, // Directly set the character_name
