...
export const fetchScriptsFull = async (req, res) => {
...
export const updateScriptCharacters = async (req, res) => {
    const { scriptId } = req.query;
    ...
    try {
        const updatedScript = await scriptsFull.findByIdAndUpdate(scriptId, { characters }, { new: true });
        if (!updatedScript) {
            return res.status(404).json({ message: "Script not found" });
        }
...
export const updateScriptContent = async (req, res) => {
    const { scriptId } = req.query;
    ...
    try {
        const updatedScript = await scriptsFull.findByIdAndUpdate(scriptId, { content: newContent }, { new: true });
        if (!updatedScript) {
            return res.status(404).json({ message: "Script not found" });
        }
        return res.status(200).json(updatedScript);
...
export const updateScriptTitlePage = async (req, res) => {
    const { scriptId } = req.query;
    ...
    try {
        const updatedScript = await scriptsFull.findByIdAndUpdate(scriptId, { title_page }, { new: true });
        if (!updatedScript) {
            return res.status(404).json({ message: "Script not found" });
        }
        return res.status(200).json(updatedScript);
...
export const createScript = async (req, res) => {
    ...
    try {
        ...
        user.scripts_id_array.push(savedScript._id);
...
export const fetchScriptsById = async (req, res) => {
    try {
        const { ids } = req.query;
        const idsArray = (typeof ids === "string" ? ids.split(",") : []).map((id) => id.trim());
